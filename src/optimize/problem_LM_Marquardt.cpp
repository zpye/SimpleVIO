...
bool Problem_LM_Marquardt::Solve(int iterations) {
...
void Problem_LM_Marquardt::ComputeParamsInit() {
    // initialize
    params_LM_.ni = 2.0;
...
void Problem_LM_Marquardt::SolveLinearSystem(bool use_shur_marg) {
    if (use_shur_marg) {
        ...
        for (size_t i = 0; i < reserve_size; ++i) {
            Hpp_schur(i, i) += params_LM_.current_lambda * Hpp_schur(i, i);
        }
        ...
        for (Eigen::Index i = 0; i < H.cols(); ++i) {
            H(i, i) += params_LM_.current_lambda * H(i, i);
        }
...
bool Problem_LM_Marquardt::IsGoodDelta() {
    return (delta_x_.norm() < 1e-30);
}

bool Problem_LM_Marquardt::IsGoodStep() {
    double scale =
        delta_x_.transpose() * (params_LM_.current_lambda *
                                    Hessian_.diagonal().cwiseProduct(delta_x_) +
                                b_);
    ...
    if (rho > 0.0 && isfinite(temp_chi2)) {
        // last step was good
        params_LM_.current_lambda =
            std::max(params_LM_.current_lambda / 9.0, 1e-7);
        ...
        params_LM_.current_lambda =
            std::min(params_LM_.current_lambda * 11.0, 1e7);
